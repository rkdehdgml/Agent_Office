...
> 구현 결과: 최종 전체 브랜치 리뷰에서 외곽 벽이 바닥 가장자리 셀과
> 정확히 겹쳐 팀장 책상·소품·캐릭터 스프라이트를 물리적으로 침범한다는
> 문제가 발견됐다. 이를 고쳐 외곽 벽을 바닥 영역(x∈[-12,12], z∈[-8,8])
> 바깥으로 한 칸 확장했다(그리드 26열×18행, 원점 `gx=cx+13, gz=cz+9`) —
> 내부 칸막이 좌표는 변경 없음. 그 결과 실제 벽 셀 수는 114개(외곽 84 +
> 내부 30, 겹침 0개)다.
